module mod_pot_01y2
   ...
   ! * masses
   real*8 :: xm1red, xm2red, xm0red, xmtot, xm1reac, xm2reac, xm1prod, xm2prod, xm1, xm0, xm2
   ...
   integer, allocatable :: indtotproc(:,:,:,:), indcanproc(:), indangproc(:), indrgridproc(:), indr1(:,:), indr2(:,:)
   ...
   !--------------------------------------------------
      ...
      print '(40("_"),/,10x,"Pot_mod",/,40("_"))'
      open(10,file='input.dat',status='old')
      radau = 0                        ! Estas variables 
      R1inf_radial_functions = 100.d0  ! están definidas como namelist
      R2inf_radial_functions = 100.d0  ! pero no aparecen en el namelist 
      No_ref_energy = 1                ! y se definen aquí a mano
      diabatic_prod_pot = 0            ! Deberían estar en el namelist
      Proj_j2_up = 0                   ! y no aquí
      read(10,nml = inputpotmass)
      print '(80("-"),/,10x,"Mass and pot determination for 01+2= ",a20,/80("-"),/)', system
      ...
      if(radau == 0) then
         print *, ' Using A + BC body fixed Jacobi coordinates'
         ! * Reactant Jacobi
         ...
         xm0red = 0.d0
      elseif(radau == 1)then
         print *, ' Using body fixed Radau(2+1) coordinates'
         xm1red = xm1
         xm2red = xm2
         xm1reac = xm1red
         xm2reac = xm2red
      ...
      print *, ' !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
      print *, '          Pot is cut at E(eV)= ',vcutmaxeV
      print *, '          above energy of ref. state'
      print *, ' !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
   ...
   !--------------------------------------------------
      ...
      &           ,sigdiat(ielec),sigatom(ielec)
         ...
         print *, ' No. of points per angle'
         do iang=1,nangu
            print *, iang,npunreal(iang)
         enddo
         call flush(6)
         
         if(nelecmax.eq.0)then
               print *, '  !!! nelec= ',nelecmax
               call flush(6)
               call MPI_BARRIER(MPI_COMM_WORLD, ierror)
               stop
         endif

         print *, '         Vmax (eV)= ',vmaxtot/conve1/eV2cm
      &        ,' while Vcutmax(eV)= ',vcutmax/conve1/eV2cm
         print *
         print *, '         Vmin (eV)= ',vmin/conve1/eV2cm
         print *
         if(radau.eq.0)then
            print *, '      at r1=rpeq (angstroms) = ',x1min
            print *, '         r2=Rgran (angstroms)= ',x2min
         elseif(radau.eq.1)then
            print *, '      at r1 (angstroms) = ',x1min
            print *, '         r2 (angstroms) = ',x2min
         endif
            
         print *, '         gam (degrees)       = ',angmin 
         print *
         print *, '    ** end potential calculation **'
         call flush(6)
      endif ! idproc.eq.0
   ...
   &  .or.nangu.ne.mangu.or.inc.ne.minc)then
         print *, ' Problem in input no. of points and incj values'
         print *, '   no correspond to those used to generate pot'
         call flush(6)
         call MPI_BARRIER(MPI_COMM_WORLD, ierror)
         stop
   endif
   ...
   & .or.dabs(rmis2-xmis2).gt.1.d-5.or.dabs(rfin2-xfin2).gt.1.d-5)then
         ...
         print *, ' Problem in input in radial intervals'
         print *, '   no correspond to those used to generate pot'
         call flush(6)
         call MPI_BARRIER(MPI_COMM_WORLD, ierror)
         stop
   ...
   do ip=0,nproc-1
      ...
      do iang_proc=1,nanguproc
         ...
         enddo
      enddo
      print *, '  in processor ',ip,'  ntotproc= ',ntotproc(ip)
   enddo  ! ip = 0,nproc-1
   ...
   print *, 
   write(6,'(40("-"))')
   print *, '  -- Energy interval  in the calculation (eV) '

   print *, '     Emin= ',vmintot/conve1/ev2cm
   ...
   if( iwrt_pot == 1)then
      print *, 
      print *, '  Write potential files to plot'
      print *, 
      call flush(6)
   ...
   print *, ' ending pot2 in proc= ',idproc
   ...
   if(i.le.ntotproc(idproc).and.i.gt.0)then
      ...
      ir1=indr1(ir,iang)
      ir2=indr2(ir,iang)
      if(ir2.gt.npun2)print *, ' in indiproc ir2 > npun2, ',ir2
      if(ir1.gt.npun1)print *, ' in indiproc ir1 > npun1, ',ir1
   else
      print *, '  i= ',i,'  > ntot= ',ntotproc(idproc)
      call flush(6)
   endif
